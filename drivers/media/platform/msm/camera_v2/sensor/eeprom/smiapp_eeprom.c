...
static int32_t smiapp_eeprom_i2c_probe(struct i2c_client *client,
...
static int32_t smiapp_eeprom_i2c_remove(struct i2c_client *client)
{
...
static const struct of_device_id smiapp_eeprom_i2c_dt_match[] = {
	{.compatible = "qcom,smiapp_eeprom"},
	{}
};

MODULE_DEVICE_TABLE(of, smiapp_eeprom_i2c_dt_match);

static const struct i2c_device_id smiapp_eeprom_i2c_id[] = {
	{ "smiapp_eeprom", (kernel_ulong_t)NULL},
	{ }
};

static struct i2c_driver smiapp_eeprom_i2c_driver = {
	.id_table = smiapp_eeprom_i2c_id,
	.probe  = smiapp_eeprom_i2c_probe,
	.remove = smiapp_eeprom_i2c_remove,
	.driver = {
		.name = "smiapp_eeprom",
		.of_match_table = smiapp_eeprom_i2c_dt_match,
	},
};

static int __init smiapp_eeprom_init_module(void)
{
	CDBG("%s E\n", __func__);
	pr_info("Enter %s at line %d\n", __func__, __LINE__);
	return i2c_add_driver(&smiapp_eeprom_i2c_driver);
}

static void __exit smiapp_eeprom_exit_module(void)
{
	i2c_del_driver(&smiapp_eeprom_i2c_driver);
}

module_init(smiapp_eeprom_init_module);
module_exit(smiapp_eeprom_exit_module);
MODULE_DESCRIPTION("SMIAPP EEPROM driver");
